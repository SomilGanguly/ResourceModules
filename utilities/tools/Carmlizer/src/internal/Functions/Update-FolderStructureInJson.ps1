...
function Update-FolderStructureInJson {
    ...
    $inputJson = Get-Content -Path "$statePath/utilities/tools/Carmlizer/src/Settings.json"
        ...
        if ($eachBasedirectory -eq $subscriptionObject.Name) {
            ...
            foreach ($folder in $leve0) {
                ...
                foreach ($eachRsFolder in $resourcesFolder) {
                    $json = @()
                    $info = '' | Select-Object SubscriptionName, ResourceGroupName, ResourceAPI_Name, Sequence, ProvisioningState
                    ...
                    $obj | ConvertTo-Json -Depth 100 | jq '.' | Out-File -FilePath "$statePath/utilities/tools/Carmlizer/src/Settings.json"
                }
            }
        }
    }
